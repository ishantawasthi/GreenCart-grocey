...
const AddProduct = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      formData.append("description", JSON.stringify(description.split(".").map(d => d.trim()).filter(d => d)));
      ...
      <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2">
            {Array(4).fill("").map((_, index) => (
              <label key={index} htmlFor={`image${index}`}>
                <input onChange={(e) => { const f = [...files]; f[index] = e.target.files[0]; setFiles(f); }} accept="image/*" type="file" id={`image${index}`} hidden />
                <img className="max-w-24 cursor-pointer border rounded" src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} alt="upload" width={100} height={100} />
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="w-full flex flex-col gap-1">
          ...
          <select onChange={(e) => setCategory(e.target.value)} value={category} id="category" className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500/40">
            ...
            {categories.map((item, idx) => (<option key={idx} value={item.path}>{item.text}</option>))}
          </select>
        </div>
        ...
        <button type="submit" className="px-8 py-2.5 bg-indigo-500 text-white font-medium rounded hover:bg-indigo-600">ADD</button>
      </form>
    </div>
  );
};
